 ...
 * 	This program is to demonstrate understanding of Event Handling using getAction() command
 * 	@author Aurko
 */
...
public class MyEventB extends JFrame
{
	// Create the buttons
	...
	public static void main( String [] args) {
		MyEventA test2 = new MyEventA();		// Test MyEventA
		...
		// Set Title of Frame
		test2.setTitle("Part 3 Using getActionCommand");
	...
	private class ClickButtonHandler implements ActionListener {
		...
		public void actionPerformed( ActionEvent ae ){
			...
			if (ae.getActionCommand().equals(ADDBUTTON)) {
				counter++;// Increment counter by 1
			}
			// If the button is SUBTRACTBUTTON
			else if (ae.getActionCommand().equals(SUBTRACTBUTTON)) {
				counter--;// Decrement counter by 1
			}
			// If the button is RESETBUTTON
			else if (ae.getActionCommand().equals(RESETBUTTON)) {
				counter = 0; // Reset the counter to 0
			}
			// If the button is QUITBUTTON
			else if (ae.getActionCommand().equals(QUITBUTTON)) {
